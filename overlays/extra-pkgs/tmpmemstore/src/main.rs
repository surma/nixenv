...
struct RunCommand {
    ...
    /// Don't check whether the peer is a descendant
    #[arg(long = "no-pid-check", default_value_t = false)]
    no_pid_check: bool,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Retrieve(cmd) => retrieve_data(cmd),
    }
}
...
fn run_command(command: RunCommand) -> Result<()> {
    let command_clone = command.clone();
    let data = read_input_data(command.input)?;
    ...
    thread::spawn(move || {
        for stream in listener.incoming() {
            if let Err(e) = handle_stream(&command_clone, &data, stream) {
...
fn handle_stream(
    ...
    let pid_check = !command.no_pid_check;
    if pid_check {
...
fn retrieve_data(command: RetrieveCommand) -> Result<()> {
    let socket_path = if let Some(socket_path) = command.socket {
        socket_path
