{ lib, ... }:
# The user half of profiles/nixos/framework.nix. Only the keyboard backlight
# lives here: the `framework_laptop::kbd_backlight` device exists on Framework
# laptops and nowhere else.
{
  # mkAfter keeps these binds behind the shared session config from
  # ./hyprland.nix, so the generated Lua file stays stable.
