...
| Chrome never offers the Minerva cert for auto-select, even though it's installed and `/etc/minerva/certificate.pem` looks fine | Stale hardcoded issuer CN list in upstream's `chrome-enrollment` module (see below) | `machines/archon/minerva.nix` overrides `AutoSelectCertificateForUrls` with `lib.mkForce` and an updated issuer list |
...
### Named gotcha: stale Minerva CA issuer list in `chrome-enrollment`

`inputs.shopify-framework.nixosModules.chrome-enrollment` builds the
`AutoSelectCertificateForUrls` Chrome policy from a hardcoded list of Minerva
subordinate CA issuer CNs (see
`modules/chrome-enrollment.nix` lines 3-6 in the upstream checkout — that
file is vendored upstream source, read-only, do not edit it):
...
Chrome's `AutoSelectCertificateForUrls` policy matches a client cert to a
site by, among other filters, an **exact** match on the issuer's `CN`. When
Shopify rotates the Minerva subordinate CA (this happened on **2026-06-12**,
issuing certs with `CN=Shopify Minerva Subordinate CA 2026`), any cert issued
under the new CA has an issuer CN the policy list doesn't know about, so
Chrome never auto-selects it. The module also exposes no option to extend or
override the list — it's fully internal to the module.

**Symptom:** the device cert is genuinely present (in Chrome's cert store /
`/etc/minerva/certificate.pem`) and otherwise valid, but Chrome never offers
it for the Minerva-gated sites, and the device shows as untrusted /
"Unauthenticated Device" as if enrollment never happened.

**Check** — compare the cert's actual issuer against what the rendered
policy allows:

```sh
openssl x509 -in /etc/minerva/certificate.pem -noout -issuer
# CN should match one of the CNs baked into:
jq '.AutoSelectCertificateForUrls' /etc/opt/chrome/policies/managed/extra.json
```

If the `-issuer` CN isn't one of the CNs in that JSON, this is why the cert
isn't being picked up — it's not a fresh-cert propagation delay (contrast
with "Chrome says Unauthenticated Device" above, which is about *timing*;
this is about the policy *never* matching, no matter how long you wait).

**Fix (implemented):** `machines/archon/minerva.nix` overrides
`programs.chromium.extraOpts."AutoSelectCertificateForUrls"` with
`lib.mkForce`, reproducing upstream's exact JSON shape (one
`builtins.toJSON { pattern; filter.ISSUER.CN; }` entry per domain × issuer)
but with an issuer list that keeps the two old CNs and adds
`"Shopify Minerva Subordinate CA 2026"`. `mkForce` is required because
upstream's module already sets this option, and NixOS option merging would
otherwise conflict rather than let us win.

This override should be **deleted once upstream's module lists the 2026 CA
itself** — it's consumer-side surgery around a genuinely stale upstream list,
not a NixOS-specific problem. **Worth reporting upstream**: every consumer of
`chrome-enrollment.nix`, not just NixOS, has been getting the same silent
failure since the 2026-06-12 rotation, since the module hardcodes the issuer
list with no override hook.
...
- Upstream issues worth raising against `shopify-framework` (i.e. against
  ...
  - The Minerva installer's `find /usr/lib /usr/lib64 -name
    ...
    one-line fix.
  - `chrome-enrollment.nix`'s hardcoded Minerva CA issuer list
    (`Shopify Minerva Subordinate CA` / `...CA2`) is stale as of the
    2026-06-12 CA rotation (new certs have `CN=Shopify Minerva Subordinate CA
    2026`), and the module exposes no option to extend it — every consumer,
    not just NixOS, silently stops auto-selecting the Minerva cert in Chrome
    after a CA rotation. See "Named gotcha: stale Minerva CA issuer list"
    above.
