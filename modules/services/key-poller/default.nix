...
let
  ...
  # Each source host needs its own remote binary paths: shopisurm is macOS
  # (nix-profile under /Users), archon is NixOS (system profile). A single
  # global path silently fails on whichever host it does not match.
  hostModule = {
    options = {
      address = mkOption {
        type = types.str;
      ...
      nuBin = mkOption {
        type = types.str;
        description = "Absolute path to nushell on this host";
      };

      gcloudBin = mkOption {
        type = types.str;
        description = "Absolute path to gcloud on this host";
      };
    };
  };

  hostsJson = builtins.toJSON (map (host: { inherit (host) address nuBin gcloudBin; }) cfg.hosts);
  ...
  keyPoller = pkgs.stdenvNoCC.mkDerivation {
    ...
    installPhase = ''
      ...
      makeWrapper ${pkgs.nushell}/bin/nu $out/bin/key-poller \
        ...
        --set-default KEY_POLLER_HOSTS_JSON ${escapeShellArg hostsJson} \
...
in
{
  options.services.key-poller = {
    ...
    sshUser = mkOption {
      type = types.str;
      default = "surma";
      description = "User to SSH as on the source hosts";
    };

    sshIdentityFile = mkOption {
      type = types.path;
      default = "/home/surma/.ssh/id_machine";
      description = "SSH identity file used to connect to the source hosts";
    };

    hosts = mkOption {
      type = types.listOf (types.submodule hostModule);
      default = [ ];
      description = ''
        Source hosts to try in order. The first host that returns a
        non-empty key wins.
      '';
    };
  ...
  config = mkIf cfg.enable {
    assertions = [
      {
        assertion = cfg.hosts != [ ];
        message = "services.key-poller.hosts must list at least one source host";
      }
    ];
    ...
    systemd.services.key-poller = {
      description = "Poll Shopify key from a source host and forward it to pylon";
      after = [
