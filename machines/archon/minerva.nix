...
in
{
  ...
  #
  #   sudo env -i PATH="$(cat /etc/minerva/script-path)" \
  #     /var/lib/fleet/minerva-agent/install-tpm-nss-deps-worker.sh
  environment.etc."minerva/script-path".text = config.systemd.services.orbit.environment.PATH;

  environment.etc."minerva/README".text = ''
